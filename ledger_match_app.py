...
def file_signature(uploaded_file):
    """A stable fingerprint for an uploaded file, used to detect when the person has
    swapped in a different file (as opposed to Streamlit just re-running the script)."""
    return getattr(uploaded_file, "file_id", None) or (uploaded_file.name, uploaded_file.size)
...
def clear_downstream_state():
    ...
    for k in ("stage1", "result", "groups", "ewb_rows", "use_party_used", "tolerance_used"):
